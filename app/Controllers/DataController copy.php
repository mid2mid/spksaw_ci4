...
class DataController extends BaseController
{
	...
	public function show()
	{
		...
		$result = $dataModel->select('data.id_data, data.id_project,data.id_kriteria,data.nama,kriteria.nama as kriteria,data.value')->join('kriteria', 'kriteria.id_kriteria = data.id_kriteria')->orderBy('data.nama', 'asc')->where(['data.id_project' => '2'])->findAll();
		d($result);
		// dd(key(array_slice($result, -1, 1, true)));
		...
		foreach ($result as $i => $v) {
			if ($dummy == $v['nama']) {
				...
				// array_push($response['data'][$index]['kriteria'], [$v['id_kriteria'] => $v['value']]);
			} else {
		...
		$result = $kriteriaModel->select('id_kriteria, nama,deskripsi,atribut, bobot');
		$result = $project ? $result->where('id_project', $project)->findAll() : $result->findAll();
	...
	public function store()
	{
		...
		if (!$this->validate([
			...
			'bobot' => 'required|is_natural_no_zero|less_than[6]',
