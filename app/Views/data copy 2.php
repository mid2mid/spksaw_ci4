...
<script>
	...
	// function htmlTbody(id, nama, kriteria,response) {
	// 	let parent = ''
	// 		parent += `<tr class="dt-${v.id}" data-data="${v.id}"><th scope="row" class="fit">${i+1}</th>`
	// 		parent += `<td class="data__nama">${v.nama}</td>`
	// 		$.each(kriteria, function(i2, v2) {
	// 			parent += `<td class="data__${i2}">${v2}</td>`
	// 		});
	// 		parent += `<td class="fit"><button type="button" class="btn btn-sm btn-outline-success btn-ubah me-2" title="ubah kriteriac"><i class="bi bi-pencil-square"></i></button><button type="button" class="btn btn-sm btn-outline-danger btn-hapus" title="hapus kriteria"><i class="bi bi-trash"></i></button></td></tr>`
	// 	$('tbody').append(parent)
	// }
	...
	function modalForm(response) {
		let html = ''
		$.each(response, function(i, v) {
			html += `
					<div class="form-floating mb-3">
						<input type="number" name="${i}" id="tambah-${i}" class="form-control" placeholder="Ketik di sini" autocomplete="off" required />
						<label>${i}</label>
					</div>`
		})
		$('#tambah-form').prepend(html)
		let html = ''
		$.each(response, function(i, v) {
			html += `
					<div class="form-floating mb-3">
						<input type="number" name="${i}" id="ubah-${i}" class="form-control" placeholder="Ketik di sini" autocomplete="off" required />
						<label>${i}</label>
					</div>`
		})
		$('#ubah-form').prepend(html)
	}

	modalForm(response[0].kriteria)
